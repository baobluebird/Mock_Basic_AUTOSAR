...
FUNC(Std_ReturnType, NvM_CODE) Nvm_Write(VAR(uint16_t, AUTOMATIC) DTC)
{
    ...
    if (MemIf_Write(MEMIF_FLASH, 0x0001, (const uint8_t *)&DTC, sizeof(uint16_t)))
    {
        return RTE_E_OK;
    }
    return RTE_E_NOT_OK;
...
{
    return MemIf_Read(MEMIF_FLASH, 0x0001, (uint8_t *)data, sizeof(uint16_t)) ? RTE_E_OK : RTE_E_NOT_OK;
}
